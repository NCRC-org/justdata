# Execute API test directly - this will be imported and run
import json
import urllib.request
import urllib.parse

print("="*80)
print("PROPUBLICA API TEST - Executing Now")
print("="*80)
...
for strein, name in test_eins:
    ...
    try:
        ...
        if data.get('organizations'):
            ...
            results.append({'ein': strein, 'organization': org, 'success': True})
        else:
            print("\n✗ No organizations found")
            results.append({'ein': strein, 'success': False})
            
    except Exception as e:
        print(f"\n✗ Error: {type(e).__name__}: {e}")
        import traceback
        traceback.print_exc()
        results.append({'ein': strein, 'error': str(e), 'success': False})
...
# Save results using path that avoids apostrophe issue
output_path = r"C:\dream\#JustData_Repo\api_test_results.json"
try:
    ...
    try:
        ...
        print(f"\n⚠ Could not save results file: {e2}")
