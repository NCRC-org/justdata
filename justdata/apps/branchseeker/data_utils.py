...
def get_last_5_years_sod() -> List[int]:
    """
    Get the last 5 years dynamically from SOD data (branches.sod and branches.sod_legacy).
    
    Returns:
        List of the 5 most recent years available, sorted descending (e.g., [2025, 2024, 2023, 2022, 2021])
    """
    try:
        client = get_bigquery_client(PROJECT_ID)
        query = f"""
        ...
        FROM (
            SELECT year FROM `{PROJECT_ID}.branches.sod` WHERE year IS NOT NULL
            UNION DISTINCT
            SELECT year FROM `{PROJECT_ID}.branches.sod_legacy` WHERE year IS NOT NULL
        )
        ORDER BY year DESC
        LIMIT 5
        """
        ...
        if years:
            print(f"✅ Fetched last 5 SOD years: {years}")
            return years
        else:
            # Fallback to recent years
            print("⚠️  No SOD years found, using fallback")
            return list(range(2021, 2026))  # 2021-2025
    except Exception as e:
        print(f"Error fetching SOD years: {e}")
        # Fallback to recent years
        return list(range(2021, 2026))  # 2021-2025
