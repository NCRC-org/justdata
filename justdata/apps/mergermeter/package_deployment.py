#!/usr/bin/env python3
"""
Package MergerMeter application for deployment.
Creates a deployment-ready ZIP file with all necessary files.
"""
...
def get_project_root():
    ...
    # This script should be in apps/mergermeter/
    script_path = Path(__file__).resolve()
    # Go up: apps/mergermeter -> apps -> root
...
def create_deployment_package():
    ...
    includes = {
        ...
        'apps/mergermeter/.env.example': 'apps/mergermeter/',
    ...
    if not deployment_readme.exists():
        # Copy from apps/mergermeter if it exists
        ...
        if source_readme.exists():
            ...
            deployment_readme.write_text(
                ...
                "See apps/mergermeter/DEPLOYMENT_PACKAGE.md for complete deployment instructions.\n"
            )
    ...
    with zipfile.ZipFile(package_zip, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, dirs, files in os.walk(package_dir):
            ...
            for file in files:
                ...
                print(f"  [OK] Added to ZIP: {arcname}")
    ...
    print(f"\n{'='*70}")
    print(f"[SUCCESS] Deployment package created successfully!")
    print(f"{'='*70}")
    print(f"Package: {package_zip}")
    print(f"Size: {package_zip.stat().st_size / 1024 / 1024:.2f} MB")
    print(f"\nFiles included: {copied_count}")
    print(f"Files skipped: {skipped_count}")
    print(f"\nNext steps:")
    print(f"1. Extract the ZIP file")
    print(f"2. Copy apps/mergermeter/.env.example to .env in root")
    print(f"3. Configure .env with your credentials")
    print(f"4. Install dependencies: pip install -r requirements.txt")
    print(f"5. Run: python run_mergermeter.py")
    print(f"{'='*70}")
