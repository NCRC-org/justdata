...
def save_bizsight_excel_report(analysis_result: Dict[str, Any], output_path: str, metadata: Dict[str, Any] = None):
    ...
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        ...
        # Create Methods & Definitions sheet (first sheet)
        notes_sheet = workbook.create_sheet(sanitize_sheet_name('Methods & Definitions'), 0)
        ...
        notes_content.append(('', 'Community Reinvestment Act Small Business Lending Data (Section 1071 / CRA disclosure), accessed through NCRC\'s BigQuery data warehouse'))
        ...
        notes_content.append(('Disclaimer', ''))
        notes_content.append(('', 'This report was generated by NCRC\'s JustData BizSight application using publicly available CRA small business lending disclosure data.'))
        notes_content.append(('', 'This analysis is provided for informational purposes and does not constitute legal or financial advice.'))
        notes_content.append(('', ''))
        ...
        for sheet_name in writer.sheets:
            if sheet_name == 'Methods & Definitions':
                continue  # Skip Methods & Definitions sheet
...
def download_excel(analysis_result, metadata):
    """Download Excel file bundled as ZIP."""
    import tempfile
    import zipfile
    import io
    import re
