...
def run_analysis(counties_str: str, years_str: str, run_id: str = None, progress_tracker=None, 
    ...
    try:
        ...
        if progress_tracker:
            print(f"[DEBUG] Updating progress to fetching_census_data", flush=True)
        ...
        try:
            ...
            if not api_key:
                ...
                print(f"  [INFO] CENSUS_API_KEY is set", flush=True)
            ...
            if counties_with_fips and len(counties_with_fips) > 0:
                ...
                print(f"  [INFO] Starting Census API calls (this may take 1-2 minutes)...", flush=True)
                import sys
                sys.stdout.flush()
        ...
        if progress_tracker:
            print(f"[DEBUG] Updating progress to processing_data", flush=True)
